...
{
	printf("USAGE:\n");
	printf("   %s eventName eventKey eventValue\n", program_invocation_short_name);
	printf("\n");
...
static int wait_event(int fd, const char* keyname, int keyvalue) {
  struct input_event ev[64];
  int i, rd;
  fd_set rdfs;
  const char* current_codename;
  ...
  while (1) {
    ...
    for (i = 0; i < rd / sizeof(struct input_event); i++) {
      if (ev[i].type == EV_KEY) {
	if(ev[i].value == keyvalue) {
	  current_codename = codename(ev[i].type, ev[i].code);
	  if(strcmp(keyname, current_codename) == 0) {
...
static int do_capture(const char* devicename, const char* keyname, int keyvalue) {
  int fd;
  ...
  int x = wait_event(fd, keyname, keyvalue);
  close(fd);
  return x;
}
...
int main (int argc, char **argv)
{
  ...
  return do_capture(devicename, keyname, keyvalue);
}
